...
public class PlayerCamp : NetworkBehaviour
{
    ...
    public override void OnStartServer()
    {
        ...
        _nbRow = gameManager.nbRow.Value;
        _nbCol = gameManager.nbCol.Value;
    ...
    {
        if (loc.Row < 0 || loc.Row >= _nbRow) return WhyItsImpossibleToPutCardOnCamp($"Row invalid : {loc.Row} not in [0, {_nbRow}]");
        if (loc.Col < 0 || loc.Col >= _nbCol) return  WhyItsImpossibleToPutCardOnCamp($"Col invalid : {loc.Col} not in [0, {_nbCol}]");
